// --- GLOBAL STATE ---
let borrowers = [], activeIndex = -1, globalPenaltyAmt = 50, globalPenaltyHrs = 5;
...
// --- DATA MANAGEMENT ---
function loadBorrowers() { const saved = localStorage.getItem("lendingBorrowers"); if (saved) borrowers = JSON.parse(saved); }
function saveBorrowersToStorage() { localStorage.setItem("lendingBorrowers", JSON.stringify(borrowers)); renderTabs(); }
...
function showCreateScreen() {
    ...
    // Also ensure Manage fields are wiped so they don't linger
    resetManageInputs();
}
...
function selectBorrower(index) {
    ...
    // Always start with a clean slate when selecting a borrower
    resetManageInputs();
}
...
function calculate() {
    ...
    const start = new Date(new Date(b.dueDate + "T00:00:00")); start.setDate(start.getDate()+1);
    ...
    if (end > start) {
        ...
        while (new Date(curr.getTime() + b.penaltyHrs*3600000) <= end) {
            ...
            count++; totalPen += b.penaltyAmt;
            let key = `${curr.getMonth()+1}/${curr.getDate()}/${curr.getFullYear().toString().slice(-2)}`;
            if(!dailyCounts[key]) dailyCounts[key]=0; dailyCounts[key]++;
            detailed += `<p style="border-bottom:1px solid #eee;font-size:0.85em;"><b>${key}:</b> ${curr.toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'})} - ${next.toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'})} <span style="float:right;">+₱${b.penaltyAmt}</span></p>`;
            curr = next;
        }
    } else isCurrentPenalty = false;

    receiptSummaryHTML = "";
    for(const [k,v] of Object.entries(dailyCounts)) receiptSummaryHTML += `${k}: ${v} blocks (₱${formatNumber(v*b.penaltyAmt)})\n`;
    ...
    out.innerHTML = `<h3 style="color:${isPartial?'#d35400':isCurrentPenalty?'#c0392b':'#27ae60'};">${isPartial?'PARTIAL PAYMENT':isCurrentPenalty?'LATE PAYMENT':'PAID ON TIME'}</h3>
    <p>Prev Balance: ₱${formatNumber(b.balance)}</p><p>Penalty: ₱${formatNumber(totalPen)}</p><hr><p><b>Total Due: ₱${formatNumber(grand)}</b></p>
    <p style="color:#27ae60;font-size:1.1em;"><b>Paying: ₱${formatNumber(paid)}</b></p>${isPartial?`<p style="color:#d35400;">Remaining: ₱${formatNumber(bal)}</p>`:''}`;
    out.style.display = "block";
    
    if(count>0){ document.getElementById("breakdown").innerHTML=detailed; document.getElementById("breakdown").style.display="block"; }
    else document.getElementById("breakdown").style.display="none";
...
async function downloadSmartReceipt() {
    ...
    // 1. Generate Image
    document.getElementById("receipt-content").innerHTML = `<h2 style="text-align:center;">OFFICIAL RECEIPT</h2><p style="text-align:center;font-size:12px;">CM's LENDING</p><hr><p><b>Date:</b> ${new Date().toLocaleString()}</p><p><b>Borrower:</b> ${name}</p><p><b>Due Date:</b> ${b.dueDate}</p><hr><pre style="white-space:pre-wrap;font-family:inherit;">${summary}</pre><hr>${isCurrentPenalty?`<p><b>PENALTY SUMMARY:</b></p><pre style="font-size:12px;white-space:pre-wrap;font-family:inherit;">${receiptSummaryHTML}</pre><hr>`:''}<p style="text-align:center;font-weight:bold;">THANK YOU!</p>`;
    ...
    if(rem <= 0) { 
        ...
        // REFRESH SCREEN: Update balance and clear inputs
        document.getElementById("manageAmount").value = formatNumber(b.balance);
        resetManageInputs(); // FORCE CLEAR
    }
}
...
// --- HELPERS ---
function formatInput(i){ let v=i.value.replace(/[^0-9.]/g,''); const p=v.split('.'); if(p.length>2)v=p[0]+'.'+p.slice(1).join(''); if(v)i.value=parseFloat(v).toLocaleString('en-US',{maximumFractionDigits:2})+(v.endsWith('.')?'.':''); else i.value=""; }
...
function copyToClipboard(){ navigator.clipboard.writeText(`📌 *CM's LENDING*\n👤 ${borrowers[activeIndex].name}\n📅 ${new Date().toLocaleDateString()}\n\n${document.getElementById("output").innerText}\n\nThank you!`).then(()=>alert("Copied!")); }
function loadSettings(){ const sA=localStorage.getItem("gA"), sH=localStorage.getItem("gH"); if(sA)globalPenaltyAmt=parseFloat(sA); if(sH)globalPenaltyHrs=parseFloat(sH); document.getElementById("setPenaltyAmount").value=globalPenaltyAmt; document.getElementById("setPenaltyHours").value=globalPenaltyHrs; }
function saveSettings(){ globalPenaltyAmt=parseFloat(document.getElementById("setPenaltyAmount").value); globalPenaltyHrs=parseFloat(document.getElementById("setPenaltyHours").value); localStorage.setItem("gA",globalPenaltyAmt); localStorage.setItem("gH",globalPenaltyHrs); document.getElementById("settingsModal").style.display="none"; }
function openSettings(){ document.getElementById("settingsModal").style.display="block"; }
window.onclick = function(e){ if(e.target==document.getElementById("settingsModal"))document.getElementById("settingsModal").style.display="none"; }